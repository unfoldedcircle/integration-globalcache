...
def check_version_match(version):
    """Check if the version matches the version in driver.json, package.json and package-lock.json."""
    base_path = os.path.join(os.path.dirname(__file__), "..")
    for filename in ["driver.json", "package.json", "package-lock.json"]:
        ...
        if file_version != version:
            print(f"Error: Provided version '{version}' does not match {filename} version '{file_version}'.")
            sys.exit(1)
